...
def efficiency_calculator(
    ...
    """
    single_frequency = int(np.size(rich_nw.nw.frequency.f)) == 1
    if not single_frequency:
        if target_f is None and rich_nw.target_f is None:
            raise ValueError("Target frequency is not set.")
        if range_f is None and rich_nw.range_f is None:
    ...
    # A single-frequency network has exactly one point to analyze; otherwise
    # select the points falling inside the requested narrow band.
    if single_frequency:
        f_indices = [0]
    else:
        f_indices = [
            ...
            if abs(rich_nw.target_f - rich_nw.nw.frequency.f[f_index])
            < rich_nw.range_f / 2
        ]
